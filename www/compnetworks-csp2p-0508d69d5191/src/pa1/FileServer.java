...
public class FileServer extends AbstractNIOServer {
	...
	protected void processAcceptedConnection(SocketChannel connChannel) {
		...
		GradingState gradingState = GradingServer.testingMap.processRequest
				(null, connChannel.socket().getInetAddress());
		if (gradingState != null)
			log.info(gradingState.getID() + " : " + gradingState.toString());
	...
	protected void executeRequest(String request, SocketChannel connChannel) {
		...
		try {
			...
			GradingState gradingState = this.updateStats(request, connChannel);
			log.info("Received " + (FileManager.isHeaderRequest(request) ?
					"HEADER" : (FileManager.getChunkNumber(request) == -1 ?
					GradingState.Keys.RECEIVED_FULL_REQUEST : GradingState.Keys
					.RECEIVED_CHUNK_REQUEST)) + " request on " +
					connChannel + ": " + request);
			if (gradingState != null)
				log.info(gradingState.getID() + " : " + gradingState.toString
						());
	...
	private GradingState updateStats(String request, SocketChannel channel) {
		GradingState state = this.updateTestingStats(request, channel);
		this.updateClientIPStats(request, channel);
		return state;
	}

	private GradingState updateTestingStats(String request, SocketChannel channel) {
		return GradingServer.testingMap.processRequest(request, channel);
	}
