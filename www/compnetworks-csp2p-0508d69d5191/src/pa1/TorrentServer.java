...
import pa1.autograding.GradingStateMap;
...
public class TorrentServer implements Runnable {
	...
	public void run() {
		while (true) {
			...
			try {
				...
				GradingState state = GradingServer.testingMap.processRequest
						(new String(
						udpRequest.getData()), udpRequest.getAddress());
				log.info(state.getID() + " : " + state.toString
						());
	...
	private String makeResponse(DatagramPacket udpRequest)
		...
		String myAddr = this.getMyAddress().getHostAddress();
		newResponse =
