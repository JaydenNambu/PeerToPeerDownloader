...
public class GradingServer extends AbstractNIOServer {
	...
	private static void send(SocketChannel connChannel, String message) {
	...
	private static boolean isRegisterRequest(String request) {
		return request.toLowerCase().trim().matches(
			Config.Commands.IAM.toString().toLowerCase() + "\\s+\\d+");
	}

	private static void processRegister(String request, SocketChannel
			connChannel) {
		...
		send(connChannel, challenge + message);

	}

	public static void processRegisterIP(SocketChannel connChannel) {
		String testingID = (connChannel.socket().getInetAddress
				().toString());
		String filename = FileManager.getChallengeFilename();
		String message = testingMap.register(filename, testingID, connChannel);
		log.info("Registered client " + testingID + ":" + testingMap
				.getTestingState(testingID));
	}

	private static String extractClientID(String request) {
